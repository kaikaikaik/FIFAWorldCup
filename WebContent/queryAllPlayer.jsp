						...
						<div id="SUBD1526869985062680">
							...
							<div class="name">
								<div class="brief" id="playerdata">
									<h2 class="page-header">球星数据列表</h2>
...
<script type="text/javascript">
			...
			function getData() {
				$.getJSON('queryAllPlayer', function(data) {
					...
					$.each(list, function(index, item) {
						...
						html += '	<td>' + item.pBirth + '</td>';
						html += '	<td>' + item.pHeight + '米</td>';
						html += '	<td>' + item.pWeight + '公斤</td>';
						html += '	<td>' + item.pNumber + '</td>';
						html += '	<td>' + item.pPosition + '</td>';
						html += '	<td>';
						html += '		<span onClick="deletePlayerById(' + item.pId + ')">删除</span>';
						html += '		<a href="queryPlayerById?pId=' + item.pId + '">修改</a>';
						html += '		<a href="addPlayer.jsp">增加</a>';
						html += '	</td>';
						html += '</tr>';
			...
			function deletePlayerById(pId) {
				if (confirm("确认删除吗？")) {
					//删除掉
					$.get("deletePlayerById", {
						pId: pId
					}, function(data) {
						if (data == 'yes') {
							alert("删除成功");
